...
const TableStudent = ( {data}:any) => {
  ...
  // TODO : student should pass via props !!
  const goToInfo =()=> {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
     <div className='mb-4 w-full flex  justify-end'>
        <button
          className="inline-flex items-center justify-center gap-2.5 rounded-md bg-meta-3 py-3 px-7  text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10"
        >
          <span>
            <svg
      ...
      <table className="w-full table-auto">
          ...
          <tbody>
            {paginatedData.map((student : any , key : any) => (
              <tr key={key}>
                {/* Profile Image */}
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex gap-2 flex-wrap">
                    {student.seances.map((done : any , index : any) => (
                      <div
                        key={index}
                      ...
                      >
                        <p className="text-center text-white text-sm">{index + 1}</p>
                      </div>
                    ))}
                  </div>
                </td>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex gap-2 flex-wrap">
                    {student.sorties.map((done : any , index : any) => (
                      <div
                        key={index}
                        className={`w-6 h-6 rounded-full ${
                          done ? 'bg-green-500' : 'bg-gray-300'
                ...
                {/* Telephone */}
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">{student.tel}</p>
                </td>
                {/* Actions */}
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                <div className="flex items-center space-x-3.5">
                    <button className="hover:text-primary" onClick={()=> goToInfo()}>
                      <svg
                        className="fill-current"
                        width="18"
                        height="18"
      ...
      <div className="flex justify-between items-center my-4 ">
        {/* <div>
          ...
          <div>
            <label htmlFor="itemsPerPage" className="mr-2 text-sm font-medium text-gray-700 dark:text-white">
              Items per page:
            </label>
            <select
              id="itemsPerPage"
              className="border border-gray-300 outline-none rounded px-2 py-1 text-sm dark:text-black dark:bg-gray-100 "
              value={itemsPerPage}
          ...
          {Array.from({ length: totalPages }, (_, index) => (
            <button
              ...
              className={`px-3 py-1 border rounded ${
                currentPage === index + 1 ? 'bg-gray-300 dark:text-black dark:bg-gray-50' : ''
              }`}
